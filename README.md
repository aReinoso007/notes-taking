...
From the repo root, one command boots **db + api + web** (Docker Desktop must be running):
...
Design reference files used during development are kept local (gitignored) and
are not part of this repo. Token values are mirrored in Python
(`CATEGORY_PALETTE`) and CSS (`styles/tokens.css`).
...
- **Design fidelity.** Local Figma SVG exports and screenshots were used for
  layout/copy; outlined SVG text was *not* used to invent typography.
